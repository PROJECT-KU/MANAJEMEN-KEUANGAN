...
@section('title')
Data Pendaftaran Scopus Camp | MIS
@stop
                    ...
                    <div class="table-responsive">
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody id="PendaftaranScopusCampTable">
                                    @php
                                    $no = 1;
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center" style="gap: 6px;">
                                                <a href="{{ route('account.pendaftaranscopuscamp.edit', $data->id) }}" class="btn btn-sm btn-warning">
...
<script>
    ...
    document.getElementById('liveSearch').addEventListener('keyup', function() {
        ...
        timer = setTimeout(() => {
            ...
            // Fetch data tabel
            fetch(`{{ route('account.pendaftaranscopuscamp.search') }}?q=${encodeURIComponent(query)}`)
                .then(response => response.text())
                .then(html => {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(html, 'text/html');
                    const newTableBody = doc.querySelector('#PendaftaranScopusCampTable');
                    if (newTableBody) {
                        document.getElementById('PendaftaranScopusCampTable').innerHTML = newTableBody.innerHTML;
                    }
                });
        }, 300);
    });
    ...
    function Delete(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/account/PendaftaranScopusCamp/delete/" + id,
                    type: 'POST',
                    data: {
                        "_token": token,
                        "_method": "DELETE"
