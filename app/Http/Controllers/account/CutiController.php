    ...
    public function index(Request $request)
    {
        ...
        if ($user->level == 'manager') {
            ...
            $Ajukan->where(function ($query) use ($user) {
                $query->where('cuti.user_id', $user->id)
                    ->whereIn('cuti.status', ['draft', 'diajukan']);
            });
        }
        ...
        return view('account.cuti.index', compact('DatasAjukan', 'DatasDiterima', 'DatasDitolak', 'startDate', 'endDate', 'countAjukan'));
    }
